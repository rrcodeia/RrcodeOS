...
Ler `templates/skills/catalogo.md` pra ver se alguma das tarefas mencionadas já é coberta por uma skill validada pelo RrcodeOS. Se sim, sugerir a skill existente em vez de criar uma nova:

> "A tarefa X já é resolvida pela skill `<nome>`. Quer ativar ela em vez de criar uma nova?"
...
**Dependências:** [arquivos do .agents/rules/, identidade/, ou ferramentas externas]
```
...
4. Calibrar o tom e regras conforme `.agents/rules/preferencias.md` e `.agents/rules/empresa.md`
...
Pra usar: peça para eu rodar a skill pelo nome em qualquer sessão.
